 ...
 */
void ATEMbase::_parseGetCommands(const char *cmdStr)	{
...
//	long temp;
	...
	if(!strcmp_P(cmdStr, PSTR("TlIn"))) {
		...
		sources = word(_packetBuffer[0],_packetBuffer[1]);
		if (getVideoSrcIndex(sources)<=46) {
			memset(atemInputShortName[getVideoSrcIndex(sources)],0,5);
			strncpy(atemInputShortName[getVideoSrcIndex(sources)], (char *)(_packetBuffer+22), 4);
			#if ATEM_debug
			if ((_serialOutput==0x80 && hasInitialized()) || (_serialOutput==0x81 && !hasInitialized()))	{
				Serial.print(F("atemInputShortName[getVideoSrcIndex(sources)=")); Serial.print(getVideoSrcIndex(sources)); Serial.print(F("] = "));
				Serial.println(atemInputShortName[getVideoSrcIndex(sources)]);
			}
			#endif
		}
	}
...
/**
	 ...
	 * videoSource 	(See video source list)
	 */
	char *  ATEMbase::getInputShortName(uint16_t videoSource) {
